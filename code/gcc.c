...
/*PP */
void process_flags_pass0(int ,char **);
void process_flags_pass1(int n,char **arg);
void replace_argv_value(int n,char **arg,char *find,char *replace);
int get_filename_and_extension(char *filename,char *name,char *ext);
void para_process( char *in,char *out);
void PP_read_input_file(int i,int argc,char **argv);
void rev( char *str);
/*PP */

/* The Specs Language
...
rev( char *str) 
{
	char *ptr=str+strlen(str)-1,temp;
...
/* Function returns 0, 
	if Filename does not end with .c 
   		else
   	extension .c is stored in ext and filename without extension is stored in name and then function returns 1 	
...
int
get_filename_and_extension(char *filename,char *name,char *ext) 
{
	
	int i,k,len=strlen(filename),temp=0;
	if( filename[len-1]!='c'||filename[len-2]!='.' ) return 0;
	for(k=0,i=len-1;filename[i]!='.';i--)
	 	ext[k++]=filename[i];
	ext[k]='\0';
...
void 
replace_argv_value(int n,char **arg,char *find,char *replace)
{
	int i;
	for(i=0;i<n;i++) {
		if(!strcmp(arg[i],find) )  {
			
			
			arg[i]=XNEWVEC(char,strlen(replace)+1);
			strcpy(arg[i],replace);
			break;
		}
	}
...
void
PP_read_input_file(int i,int argc,char **argv) 
{
	char name[2550],ext[2550];
	char outputFileName[2550];

	if( get_filename_and_extension(infiles[i].name,&name,&ext)) {
		strcpy(outputFileName,strcat(strcat(name,"_parallel."),ext));
		//parallelize(fin,fout);
		para_process(infiles[i].name,outputFileName); //lex_yacc prog
		replace_argv_value(argc,argv,infiles[i].name,outputFileName);
	}
}
...
void read_input_files(int argc,char **argv) {
	...
	for (i = 0; (int) i < n_infiles; i++) {
		...
		PP_read_input_file(i,argc,argv);
	}
}

void 
process_flags_pass0(int n,char **arg) 
{
	int i;	
	for(i=0;i<n; i++) {
		if(!strcmp(arg[i],"-fparallel"))
		{
			fparallel_flag=1;
				
			arg[i]=XNEWVEC(char,strlen("-c")+1);
			strcpy(arg[i],"-c");
		}
		
		if(strstr(arg[i],"-fnested")) {
			flag_nested=(int)arg[i][9]-'0';
			
			strcpy(arg[i],"-c");
		}
		if(!strcmp(arg[i],"-finteractive"))
		{
			flag_interactive=1;
			strcpy(arg[i],"-c");
...
/* Function which replace "-c" in arg with "-fopenmp" */
void 
process_flags_pass1(int n,char **arg) {
	...
	for(i=0;i<n; i++) 
		if(!strcmp(arg[i],"-c"))
		{
			arg[i]=XNEWVEC(char,strlen("-fopenmp")+1);
			strcpy(arg[i],"-fopenmp");
		}
  ...
  for (i = 0; (int) i < n_infiles; i++)
    {
      ...
      /* PP start
		...
		if(copy_flag())
			if(fparallel_flag && pass_count==1)
				PP_read_input_file(i);
...
return_status= (signal_count != 0 ? 2
	  : error_count > 0 ? (pass_exit_codes ? greatest_status : 1)
	  : 0);
	  
	  pass_count++;
	
		if(!return_status && fparallel_flag && pass_count==1) {
			read_input_files(argc,argv);
			process_flags_pass1(argc,argv);
		    fparallel_flag=0;
			main(argc,argv);
		}  
